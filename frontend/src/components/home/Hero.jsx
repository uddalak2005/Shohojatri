...
// ── Hero ──────────────────────────────────────────────────────────────────
const Hero = ({ data, videoUrl, founderImage }) => {
  const sectionRef = useRef(null);
  const isInView = useInView(sectionRef, { once: true, margin: '-5%' });

  // Use props data or fallback to static heroData
  const displayData = data || heroData;
        ...
        <div className="lg:col-span-6 flex flex-col items-start">
          ...
          >
            ...
            <span style={{ fontFamily: 'var(--font-body)', fontSize: '11px', fontWeight: 500, color: '#2e6a4f', letterSpacing: '0.14em', textTransform: 'uppercase' }}>
              {displayData.badge || 'A Debangana Bhattacharya Initiative'}
            </span>
          </motion.div>
          ...
          <div className="flex flex-col gap-0 overflow-hidden mb-6">
            {[
              { text: displayData.titleLine1 || 'Walking Together', italic: true, delay: 0.2, size: 'clamp(40px, 6vw, 80px)' },
              { text: displayData.titleLine2 || 'Toward Wellbeing', italic: false, delay: 0.32, size: 'clamp(28px, 4.5vw, 56px)' },
          ...
          >
            {displayData.description || `Shohojatri is a psychosocial welfare initiative dedicated to walking alongside individuals and communities toward mental wellbeing, resilience, and emotional empowerment.`}
          </motion.p>
